module MCollective
  module Util
    ...
    # policy default deny
    # allow    group=devs status enable disable   country=uk     apache
    # allow    group=ops   *                       *              *
    #
    # This will deny almost all service agent requests, but allows caller
    # with group name devs to invoke the 'status,' 'enable,' and 'disable' actions on
    # nodes which have the 'country=uk' fact and the 'apache' class.
    # Unix users of group ops will be able to do all actions regardless of facts and classes.
    #
